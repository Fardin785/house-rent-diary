...
import { Home, Users, FileText, Calculator, Menu, X, Download, Globe, BookOpen, Zap } from "lucide-react";
    ...
    <div id="app" className="flex min-h-screen w-full overflow-x-hidden">
      ...
      >
        ...
        <nav className="flex-1 p-4 px-3 flex flex-col gap-1">
          ...
          <Link
            to="/electricity-calculator"
            className={`flex items-center gap-3 px-4 py-3 rounded-sm text-[0.93rem] font-medium transition-all duration-200 ${
              currentPath.startsWith("/electricity-calculator")
                ? "bg-accent-glow text-accent-hover [&_svg]:text-accent"
                : "text-text-secondary hover:bg-hover hover:text-text-primary"
            }`}
          >
            <Zap size={20} />
            <span>{t("nav.electricityCalc")}</span>
          </Link>
          <Link
            to="/family-ledger"
            className={`flex items-center gap-3 px-4 py-3 rounded-sm text-[0.93rem] font-medium transition-all duration-200 ${
      ...
      <main
        id="main-content"
        className="ml-[var(--sidebar-width)] flex-1 p-8 min-h-screen max-md:ml-0 max-md:p-4 max-md:pt-[calc(var(--mobile-header-height)+16px)]"
      >
        {children}
      </main>
    </div>
